...
void DM9000_SendPacket(struct pbuf *p)
{
  ...
  xSemaphoreTake(dm9000lock,portMAX_DELAY);//获取互斥信号量 
	...
	xSemaphoreGive(dm9000lock);		//释放互斥信号量
}
//DM9000接收数据包
...
//	OSMutexPend(dm9000lock,0,&err); 				//请求互斥信号量,锁定DM9000
    xSemaphoreTake(dm9000lock,portMAX_DELAY);
__error_retry:	
...
void DMA9000_ISRHandler(void)
{
	...
	{  
       xSemaphoreGiveFromISR(dm9000input,&pxHigherPriorityTaskWoken);//释放信号量
